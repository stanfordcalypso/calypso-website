...
if ($action != "none") {
 ...
 if (haveinput('id')) {
  ...
  if (haveinput('gigid')) {
    $result = mysql_query("SELECT members.sunetid, members.name, responses.loading, responses.playing, responses.cleanup, responses.car, responses.comments, gigs.gigid, gigs.name FROM responses, members, gigs WHERE responses.gigid = '$input[gigid]' AND responses.gigid = gigs.gigid AND responses.sunetid = members.sunetid ORDER BY members.name");
    echoresult($result);
  }
}
